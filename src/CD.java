...
public class CD extends Disco implements LecturaDisco {
    private static int id = 1;

    CD(int capacidad){
        super("CD", id, capacidad);
        id++;
    }
    ...
    public String almacenarDatos(String dato) {
        if (capacidadUsada != capacidad){
            giroDisco();
            System.out.println("Almacenando en el CD mediante el laser...");
        ...
        }else return "El CD esta completo y no puede almacenar mas datos";
    ...
    public String buscarDatos(String dato) {
        ...
        if (encontrado){return "El valor \""+dato+"\" se encuentra en la posición "+i+" del CD";}
        else return "No se encuentra el valor \""+dato+"\" no se ha encontrado en el CD";
    }
